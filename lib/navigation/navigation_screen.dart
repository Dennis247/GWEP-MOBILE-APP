...
class _NavigationScreenState extends State<NavigationScreen>
  ...
  Widget build(BuildContext context) {
    Widget? nextManeuverWidget =
        _reroutingInProgress ? null : _buildNextManeuver(context);
  ...
  void _enableTracking(bool enable) {
    setState(() {
      _visualNavigator.cameraBehavior =
          enable ? Navigation.DynamicCameraBehavior() : null;
    });
  }

  void _createMapRoute() {
    _mapRoute = MapPolyline(_currentRoute.geometry, UIStyle.routeLineWidth,
  ...
  void _startSimulatedLocations() {
    Navigation.LocationSimulatorOptions options =
        Navigation.LocationSimulatorOptions()
    ...
    if (_currentRoute.requestedTransportMode !=
      ...
      _visualNavigator.speedLimitListener =
        ...
        if (_currentSpeedLimit != speedLimit.speedLimitInMetersPerSecond) {
          setState(() =>
              _currentSpeedLimit = speedLimit.speedLimitInMetersPerSecond);
        }
      });
  ...
  void _setupVoiceTextMessages() async {
    ...
    _visualNavigator.maneuverNotificationListener =
        Navigation.ManeuverNotificationListener((text) {
      if (_soundEnabled) {
  ...
  void _setupLogoAndPrincipalPointPosition() {
    _hereMapController.setWatermarkPlacement(
