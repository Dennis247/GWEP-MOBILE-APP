...
class PositioningEngine {
  static final ConsentEngine? _consentEngine =
      Platform.isAndroid ? ConsentEngine() : null;
  ...
  Future _initialize(BuildContext context) async {
    ...
    if (!await _askPermissions()) {
      _locationEngineStatusController
          .add(LocationEngineStatus.missingPermissions);
    }

    Timer.periodic(
        Duration(seconds: 3), (timer) => _checkLocationServicesStatus());
  }
  ...
  void _createAndInitLocationEngine() {
    ...
    _locationEngine!.addLocationListener(LocationListener(
        (location) => _locationUpdatesController.add(location)));
