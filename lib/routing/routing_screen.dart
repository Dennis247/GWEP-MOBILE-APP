...
class _RoutingScreenState extends State<RoutingScreen>
  ...
  Widget build(BuildContext context) => Stack(
        children: [
          Scaffold(
            ...
            bottomNavigationBar:
                _mapInitSuccess ? _buildBottomNavigationBar(context) : null,
  ...
  void _pickMapItem(Point2D touchPoint) {
    _hereMapController.pickMapItems(touchPoint, _kTapRadius,
        ...
        switch (result) {
          ...
          case PlaceDetailsPopupResult.addToRoute:
            _wayPointsController.insert(
                _appropriateIndexToInsertWaypoint(wp), wp);
            break;
        }
      ...
      _routesTabController
          .animateTo(_mapRoutes.indexOf(mapPolyLinesList.first));
    });
  }
  ...
  int _appropriateIndexToInsertWaypoint(WayPointInfo wayPointInfo) {
    ...
    final int indexOnRoute =
        routeLine.getNearestIndexTo(wayPointInfo.coordinates);
  ...
  _updateSelectedRoute() {
    ...
    _mapRoutes[_routesTabController.index].lineColor =
        UIStyle.selectedRouteColor;
    _mapRoutes[_routesTabController.index].outlineColor =
              ...
              child: Padding(
                ...
                child: SvgPicture.asset(
                  appPreferences.showTrafficLayers
                      ? "assets/traffic_off.svg"
                      : "assets/traffic_on.svg",
    ...
    return BottomAppBar(
      ...
      child: Column(
        ...
        children: [
          ...
          //           _routePoiHandler
          //               .updatePoiForRoute(_routes[_selectedRouteIndex]);
          //         }
          //       },
          //     ),
          //     Spacer(),
  ...
  _beginRouting() {
    ...
    RoutePreferencesModel preferences =
        Provider.of<RoutePreferencesModel>(context, listen: false);

    switch (_transportModes[_transportModesTabController.index]) {
      case TransportModes.car:
        _routingEngine.calculateCarRoute(
            _wayPointsController.value, preferences.carOptions, _onRoutingEnd);
        break;
      case TransportModes.truck:
        _routingEngine.calculateTruckRoute(_wayPointsController.value,
            preferences.truckOptions, _onRoutingEnd);
        break;
      case TransportModes.scooter:
        _routingEngine.calculateScooterRoute(_wayPointsController.value,
            preferences.scooterOptions, _onRoutingEnd);
        break;
      case TransportModes.walk:
        _routingEngine.calculatePedestrianRoute(_wayPointsController.value,
            preferences.pedestrianOptions, _onRoutingEnd);
        break;
    }
  }
  ...
  _onRoutingEnd(Routing.RoutingError? error, List<Routing.Route>? routes) {
    ...
    WidgetsBinding.instance
        .addPostFrameCallback((timeStamp) => _zoomToRoutes());
  }
  ...
  void _awaitOptionsFromPreferenceScreen(BuildContext context) async {
    ...
    setState(() => _transportModesTabController.index =
        max(_transportModes.indexOf(activeTransportMode), 0));
    _beginRouting();
  }
}
